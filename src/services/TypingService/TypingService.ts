...
export class TypingService {
  ...
  async simulateRealisticTyping(
    ...
    try {
      ...
      throw error instanceof TypingError ? error : new TypingError('Realistic typing simulation failed', error as Error);
    }
  }
  ...
  private getRandomDelay(speedMultiplier: number): number {
    const baseDelay = TimingConstants.TYPING_SPEEDS.MIN_DELAY + 
      Math.random() * (TimingConstants.TYPING_SPEEDS.MAX_DELAY - TimingConstants.TYPING_SPEEDS.MIN_DELAY);
