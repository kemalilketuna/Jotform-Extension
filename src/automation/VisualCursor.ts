import { CursorPosition, VisualAnimationConfig, VisualCursorState } from '../types/AutomationTypes';
...
export class VisualCursor {
    ...
    show(position?: CursorPosition): void {
        ...
        this.logger.debug(`Cursor shown at position (${this.state.position.x}, ${this.state.position.y})`, 'VisualCursor');
    }
    ...
    updateConfig(config: Partial<VisualAnimationConfig>): void {
        ...
        this.logger.debug('Visual cursor config updated', 'VisualCursor', { config: this.config });
    }
     ...
     */
    private async animateToPosition(targetPosition: CursorPosition): Promise<void> {
