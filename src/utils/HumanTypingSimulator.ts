...
export class HumanTypingSimulator {
  ...
  private static readonly TYPO_PROBABILITY = 0.02; // 8% chance of typo
  ...
  static async simulateTyping(
    ...
    for (let i = 0; i < characters.length; i++) {
      ...
      const shouldMakeTypo = Math.random() < this.TYPO_PROBABILITY &&
      ...
      if (shouldMakeTypo && i > 2) {
        ...
        await this.wait(this.TYPING_SPEEDS.PAUSE_DELAY / speedMultiplier);
        ...
        for (let k = 0; k < backspaceCount; k++) {
          ...
          this.updateElementValue(element, currentText);
          onProgress?.(currentText);
          await this.wait(this.TYPING_SPEEDS.CORRECTION_DELAY / speedMultiplier);
        }
  ...
  static async simulateRealisticTyping(
    ...
    for (let i = 0; i < characters.length; i++) {
      ...
      const delay = shouldPause
        ? this.TYPING_SPEEDS.PAUSE_DELAY / speedMultiplier
