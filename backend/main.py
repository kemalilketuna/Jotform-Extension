            ...
            try:
                ...
                if message_type == "get_automation_sequence":
                    ...
                    if sequence_type == "form_creation":
                        sequence = (
                            AutomationSequenceGenerator.get_form_creation_sequence()
                        )
                    elif sequence_type == "form_building":
                        sequence = (
                            AutomationSequenceGenerator.get_form_building_sequence()
                        )
                    else:
                        sequence = AutomationSequenceGenerator.get_custom_sequence(
                            sequence_type, parameters
                        )
                    ...
                    logger.info(
                        f"Automation status update: {status} for sequence {sequence_id}"
                    )
                    ...
                    await manager.send_personal_message(
                        json.dumps(status_response), websocket
                    )
