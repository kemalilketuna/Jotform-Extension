...
export class WebSocketService {
   ...
   */
  addConnectionListener(listener: (state: string, error?: Error) => void): void {
   ...
   */
  removeConnectionListener(listener: (state: string, error?: Error) => void): void {
  ...
  async connect(): Promise<void> {
    ...
    this.connectionPromise = new Promise((resolve, reject) => {
      try {
        this.logger.info(`Connecting to WebSocket: ${this.config.url}`, 'WebSocketService');
        ...
        this.websocket.onopen = () => {
          ...
          this.notifyConnectionStateChange('connected');
          this.startHeartbeat();
          this.logger.info('WebSocket connected successfully', 'WebSocketService');
          resolve();
        };
        ...
        this.websocket.onclose = (event) => {
          clearTimeout(connectionTimeout);
          this.stopHeartbeat();
          this.logger.warn(`WebSocket closed: ${event.code} - ${event.reason}`, 'WebSocketService');
          
          if (!this.isManualDisconnect && !event.wasClean && this.reconnectAttempts < this.config.reconnectAttempts) {
        ...
        this.websocket.onerror = (error) => {
          clearTimeout(connectionTimeout);
          const wsError = new WebSocketConnectionError('Failed to connect to WebSocket server');
        ...
        const wsError = new WebSocketConnectionError('Failed to create WebSocket');
        this.lastError = wsError;
   ...
   */
  private async sendRequest<T>(message: WebSocketMessage, responseType: string, timeout = 10000): Promise<T> {
    const requestId = `${message.type}_${Date.now()}_${Math.random()}`;
    
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        this.pendingRequests.delete(requestId);
        reject(new WebSocketTimeoutError(`Request timeout for ${message.type}`));
  ...
  private handleMessage(data: string): void {
    try {
      ...
      this.logger.debug(`Received message: ${message.type}`, 'WebSocketService');

      const handler = this.messageHandlers.get(message.type);
      if (handler) {
        handler(message);
      } else {
        this.logger.warn(`No handler for message type: ${message.type}`, 'WebSocketService');
      }
    } catch (error) {
      this.logger.logError(
  ...
  private scheduleReconnect(): void {
    if (this.isManualDisconnect || this.reconnectAttempts >= this.config.reconnectAttempts) {
      if (this.reconnectAttempts >= this.config.reconnectAttempts) {
        this.logger.error('Max reconnection attempts reached', 'WebSocketService');
        this.connectionState = 'failed';
        this.notifyConnectionStateChange('failed', new WebSocketConnectionError('Max reconnection attempts reached'));
      }
      return;
    }
  ...
  async fetchFormCreationSteps(): Promise<AutomationSequenceResponse> {
    try {
      this.logger.info('Fetching form creation steps from WebSocket server', 'WebSocketService');
      ...
      this.logger.info('Form creation steps fetched successfully', 'WebSocketService');
      return response;
    } catch (error) {
      this.logger.logError(error as Error, 'WebSocketService');
  ...
  async fetchFormBuildingSteps(): Promise<AutomationSequenceResponse> {
    try {
      this.logger.info('Fetching form building steps from WebSocket server', 'WebSocketService');
      ...
      this.logger.info('Form building steps fetched successfully', 'WebSocketService');
      return response;
    } catch (error) {
      this.logger.logError(error as Error, 'WebSocketService');
      throw new AutomationError(UserMessages.ERRORS.SERVER_CONNECTION_FAILED);
   ...
   */
  convertToAutomationSequence(response: AutomationSequenceResponse): AutomationSequence {
    try {
      this.logger.debug('Converting WebSocket response to automation sequence', 'WebSocketService');
      ...
      this.logger.debug('WebSocket response converted successfully', 'WebSocketService');
      return sequence;
    } catch (error) {
      this.logger.logError(error as Error, 'WebSocketService');
      throw new AutomationError('Failed to convert WebSocket response to automation sequence');
    }
  }
   ...
   */
  async sendAutomationStatus(sequenceId: string, status: 'started' | 'completed' | 'failed' | 'in_progress'): Promise<void> {
    try {
      ...
      this.logger.info(`Sent automation status: ${status} for sequence ${sequenceId}`, 'WebSocketService');
    } catch (error) {
      this.logger.logError(error as Error, 'WebSocketService');
   ...
   */
  isConnected(): boolean {
    return this.websocket?.readyState === WebSocket.OPEN && this.connectionState === 'connected';
  }
  ...
  getConnectionHealth(): {
    ...
    if (this.reconnectAttempts > 0) {
      ...
      if (this.reconnectAttempts >= this.config.reconnectAttempts) {
        recommendations.push('Maximum reconnection attempts reached - manual intervention required');
      }
    }
